...
회원 전용(비공개) 게시판은 로그인 쿠키(NAVER_COOKIE)를 넣어야 동작합니다.
"""
import requests
...
def fetch_articles(club_id: str, menu_id: str, per_page: int = 50, cookie: str = ""):
    """최신 글 목록을 [{id, title, writer, url}] 형태로 반환합니다.

    menu_id 가 "0" 이면 카페 전체글을 가져옵니다.
    cookie 가 주어지면(비공개 카페) 로그인 상태로 조회합니다.
    """
    params = {
